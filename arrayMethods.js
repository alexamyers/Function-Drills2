...
//ARROW
const postTaxesPrices = prices.map(element => element * 1.07)
...
//DECLARATION
const myStrongest = monstersInYourPocket.filter(function(monster.CP) {
  if(monster.CP > 200) {
    return monster
  }
})
...
//DECLARATION 
const orderTotal = orders.map(function(elem) {
  return elem.price * (elem.tax + 1)
})

//ARROW
const orderTotal = orders.map(elem => elem.price * (elem.tax + 1))
...
//ARROW
const bobsTotal = purchases.filter(elem => elem.owner === "Bob").reduce((acc, elem) => acc + elem.price, 0)
